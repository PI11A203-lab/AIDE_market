// db/add-tags-columns.js - Tags 테이블에 누락된 컬럼 추가
require('dotenv').config();
const models = require('./initializer');

async function addTagsColumns() {
    try {
        console.log('🔧 Tags 테이블에 누락된 컬럼 추가 중...\n');
        ...
        for (const col of requiredColumns) {
            ...
            try {
                const alterQuery = `ALTER TABLE Tags ADD COLUMN ${col.name} ${col.type}`;
