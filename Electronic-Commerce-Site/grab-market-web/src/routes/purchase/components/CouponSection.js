  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-6 mt-4">
      <div className="flex items-center gap-2.5 mb-4">
        ...
        <h3 className="text-base font-semibold text-gray-900">{t('purchase.coupon.title')}</h3>
      </div>
      <div className="flex gap-3 items-stretch">
        <input
          ...
          placeholder={t('purchase.coupon.placeholder')}
      ...
      {appliedCoupon && (
        <div className="mt-4 px-4 py-3 bg-green-100 border border-green-300 rounded-[10px] text-sm font-semibold text-green-800 flex items-center gap-2">
          ...
          <span>{t('purchase.coupon.applied', { label: appliedCoupon.label })}</span>
        </div>
      )}
    </div>
  );
